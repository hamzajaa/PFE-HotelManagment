...
@Service
public class HouseKeepingEmployeeServiceImpl extends AbstractServiceImpl<HouseKeeping> implements HouseKeepingEmployeeService {
...
private HouseKeepingDao houseKeepingDao;
    ...
    private ArchivableService<HouseKeeping> archivableService;
        @Autowired
        private EmployeeEmployeeService employeeService ;
        @Autowired
        private RoomTypeEmployeeService roomTypeService ;
        @Autowired
        private FloorEmployeeService floorService ;
        @Autowired
        private HouseKeepingStatutEmployeeService houseKeepingStatutService ;
        @Autowired
        private RoomEmployeeService roomService ;
...
public List<HouseKeeping> findAll(){
        return houseKeepingDao.findAll();
}
...
public HouseKeeping findByIdWithAssociatedList(Long id){
    ...
    @Override
    public HouseKeeping archiver(HouseKeeping houseKeeping) {
    if (houseKeeping.getArchive() == null) {
    houseKeeping.setArchive(false);
    }
    houseKeeping.setArchive(true);
    ...
    @Override
    public HouseKeeping desarchiver(HouseKeeping houseKeeping) {
    if (houseKeeping.getArchive() == null) {
    houseKeeping.setArchive(false);
    }
    houseKeeping.setArchive(false);
