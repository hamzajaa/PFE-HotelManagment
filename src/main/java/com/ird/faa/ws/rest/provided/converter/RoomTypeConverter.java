...
public class RoomTypeConverter extends AbstractConverter<RoomType, RoomTypeVo> {
    ...
    public RoomType toItem(RoomTypeVo vo) {
        if (vo == null) {
            ...
            if (StringUtil.isNotEmpty(vo.getShortCode()))
                item.setShortCode(vo.getShortCode());
            if (StringUtil.isNotEmpty(vo.getSlug()))
                item.setSlug(vo.getSlug());
            ...
            if (StringUtil.isNotEmpty(vo.getBasePrice()))
                item.setBasePrice(NumberUtil.toBigDecimal(vo.getBasePrice()));
    ...
    public RoomTypeVo toVo(RoomType item) {
        if (item == null) {
            ...
            if (StringUtil.isNotEmpty(item.getShortCode()))
                vo.setShortCode(item.getShortCode());

            if (StringUtil.isNotEmpty(item.getSlug()))
                vo.setSlug(item.getSlug());

            if (StringUtil.isNotEmpty(item.getDescription()))
                vo.setDescription(item.getDescription());

            if (item.getBaseOccupancy() != null)
                vo.setBaseOccupancy(NumberUtil.toString(item.getBaseOccupancy()));
            ...
            if (item.getKidsOccupancy() != null)
                vo.setKidsOccupancy(NumberUtil.toString(item.getKidsOccupancy()));

            if (item.getBasePrice() != null)
                vo.setBasePrice(NumberUtil.toString(item.getBasePrice()));

            if (item.getAdditionalPersonPrice() != null)
                vo.setAdditionalPersonPrice(NumberUtil.toString(item.getAdditionalPersonPrice()));

            if (item.getExtraBedPrice() != null)
                vo.setExtraBedPrice(NumberUtil.toString(item.getExtraBedPrice()));
