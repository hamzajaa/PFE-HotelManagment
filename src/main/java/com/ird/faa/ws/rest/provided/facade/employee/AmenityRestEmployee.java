...
import com.ird.faa.service.employee.facade.AmenityEmployeeService;
...
@RestController
@RequestMapping("api/employee/amenity")
public class AmenityRestEmployee {

@Autowired
private AmenityEmployeeService amenityService;
...
private AmenityConverter amenityConverter;
    ...
    public List<AmenityVo> findByCriteria(@RequestBody AmenityVo amenityVo){
        return amenityConverter.toVo(amenityService.findByCriteria(amenityVo));
            ...
            @PutMapping("/archiver/")
            public AmenityVo archiver(@RequestBody AmenityVo amenityVo){
                Amenity amenity = amenityService.archiver(amenityConverter.toItem(amenityVo));
                return amenityConverter.toVo(amenity);
                }

            @PutMapping("/desarchiver/")
            public AmenityVo desarchiver(@RequestBody AmenityVo amenityVo){
                Amenity amenity = amenityService.desarchiver(amenityConverter.toItem(amenityVo));
                return amenityConverter.toVo(amenity);}
            }
