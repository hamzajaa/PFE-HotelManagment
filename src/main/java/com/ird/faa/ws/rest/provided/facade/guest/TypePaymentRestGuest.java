...
import com.ird.faa.service.guest.facade.TypePaymentGuestService;
...
@RestController
@RequestMapping("api/guest/typePayment")
public class TypePaymentRestGuest {

@Autowired
private TypePaymentGuestService typePaymentService;
...
private TypePaymentConverter typePaymentConverter;
    ...
    public List<TypePaymentVo> findByCriteria(@RequestBody TypePaymentVo typePaymentVo){
        return typePaymentConverter.toVo(typePaymentService.findByCriteria(typePaymentVo));
            ...
            @PutMapping("/archiver/")
            public TypePaymentVo archiver(@RequestBody TypePaymentVo typePaymentVo){
                TypePayment typePayment = typePaymentService.archiver(typePaymentConverter.toItem(typePaymentVo));
                return typePaymentConverter.toVo(typePayment);
                }

            @PutMapping("/desarchiver/")
            public TypePaymentVo desarchiver(@RequestBody TypePaymentVo typePaymentVo){
                TypePayment typePayment = typePaymentService.desarchiver(typePaymentConverter.toItem(typePaymentVo));
                return typePaymentConverter.toVo(typePayment);}
            }
